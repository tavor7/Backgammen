...
export function chooseComputerMove(board: BoardState, player: Player, dice: [number, number], difficulty: Difficulty): MoveSequence {
  ...
  const deepScored = shortlist.map(({ candidate }) => ({
    candidate,
    net: candidate.score - expectedBestReplyScoreDeep(candidate.resultingBoard, opponent(player), 2),
  }));
  ...
  // search, at real computational cost, so it's reserved for a very short final list, with sample/
  // ply counts sized to keep total move time reasonable in practice.
  ...
  for (const { candidate } of rolloutShortlist) {
    const equity = -rolloutEquity(candidate.resultingBoard, opponent(player), 5, 20);
